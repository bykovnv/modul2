...
$this->layout('layout', ["title" => "Все пользователи", "auth" => $auth, "users" => $users,  ]); 
/* 
* Проверка пользователя, если он не Admin, то у него нет доступа к этой странице 
* Передресация на страницу пользователя
*/
if(!$canEditUsers == true){
    flash()->error('У вас нет доступа к редактированию пользователей');
    header("Location: /profile/" . $auth->getUserId());
    die;
}
?>
